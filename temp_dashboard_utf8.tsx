...
export default function AthleteDashboard() {
  ...
  const calculateMetrics = (evalData?: Assessment) => {
    ...
    const fatorRaca = athlete?.race === 'Negro' ? 1.1 : athlete?.race === 'Asi├ítico' ? -2 : 0;
    ...
    let massaMuscular = 0;
    // S├│ calcula massa muscular se tiver altura e pelo menos uma circunfer├¬ncia v├ílida
  ...
  const renderRelationCards = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {renderRow('Coxa', 'F├¬mur', r.ccCoxa, r.diamJoelho, r.coxa)}
        {renderRow('Panturrilha', 'Tornozelo', r.ccPantu, r.diamTornozelo, r.pantu)}
        {renderRow('Bra├ºo', '├Ümero', r.ccBraco, r.diamPunho, r.braco)}
      </div>
    );
  };
      ...
      <table className="data-table">
        <thead>
          <tr>
            <th>Medida</th>
            <th>Atual</th>
            {compareEval && <th>Compara├º├úo</th>}
            {compareEval && <th>Evolu├º├úo</th>}
          </tr>
        </thead>
        <tbody>
          {items.map((item, idx) => (
    ...
    <div className="container dashboard-container">
      {athletes.length === 0 ? (
        <Card style={{ textAlign: 'center', padding: '4rem 2rem', marginTop: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', maxWidth: '400px', margin: '0 auto 1rem', lineHeight: 1.5 }}>
            Para visualizar o dashboard e acompanhar as avalia├º├Áes antropom├®tricas, voc├¬ precisa primeiro cadastrar um atleta no sistema.
          </p>
      ...
      {!athlete ? (
        <Card style={{ textAlign: 'center', padding: '4rem 2rem', marginTop: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', maxWidth: '400px', margin: '0 auto 1rem', lineHeight: 1.5 }}>
            Utilize o menu acima para selecionar um atleta e visualizar seu dashboard de evolu├º├úo antropom├®trica.
          </p>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
            <Link to="/add" className="btn btn-primary" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <Plus size={20} />
        ...
        <>
          ...
          <Card className="athlete-header-card">
            <div className="athlete-profile-section">
              ...
              <div className="athlete-info-section">
                ...
                <p className="athlete-subtitle">Avalia├º├úo Antropom├®trica</p>
            ...
            <div className="athlete-eval-summary">
              {assessments.length > 0 ? (
                <>
                  <div className="eval-item">
                    ...
                    <div className="eval-data">
                      <span className="eval-label">AVALIA├ç├âO ATUAL</span>
                      <select
                        value={currentEvalId}
                  ...
                  <div className="eval-item">
                    ...
                    <div className="eval-data">
                      <span className="eval-label">F├ôRMULA %G</span>
                      <select
                ...
                <div className="no-evals-message">
                  <p>Nenhuma avalia├º├úo cadastrada para este atleta.</p>
                </div>
              )}
              <div className="eval-actions" style={{ marginTop: '0.5rem', width: '100%', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                >
                  <ClipboardList size={18} /> Ver Avalia├º├Áes
                </button>
            ...
            <div className="dashboard-content-layout">
              ...
              <div className="dashboard-metrics-column">
                <div className="tabs-header" style={{ marginBottom: '1rem', backgroundColor: 'transparent', padding: 0 }}>
                  <button className={`tab-btn ${metricsPage === 1 ? 'active' : ''}`} onClick={() => setMetricsPage(1)}>Composi├º├úo Corporal</button>
                  <button className={`tab-btn ${metricsPage === 2 ? 'active' : ''}`} onClick={() => setMetricsPage(2)}>├ìndices de Simetria</button>
                  <button className={`tab-btn ${metricsPage === 3 ? 'active' : ''}`} onClick={() => setMetricsPage(3)}>Rel. Cineantropom├®trica</button>
                </div>
                ...
                <Card className="data-table-card" style={{ height: '100%' }}>
                  ...
                  <div className="tab-content">
                    ...
                    {activeTab === 'dobras' && renderTableContent([
                      { label: 'Tr├¡ceps Dir.', cur: currentEval?.skinfolds?.tricepsRight, cmp: compareEval?.skinfolds?.tricepsRight, unit: 'mm', inverseGood: true },
                      { label: 'Tr├¡ceps Esq.', cur: currentEval?.skinfolds?.tricepsLeft, cmp: compareEval?.skinfolds?.tricepsLeft, unit: 'mm', inverseGood: true },
                      { label: 'Subescapular', cur: currentEval?.skinfolds?.subscapular, cmp: compareEval?.skinfolds?.subscapular, unit: 'mm', inverseGood: true },
                      { label: 'T├│rax', cur: currentEval?.skinfolds?.chest, cmp: compareEval?.skinfolds?.chest, unit: 'mm', inverseGood: true },
                      { label: 'Subaxilar', cur: currentEval?.skinfolds?.midaxillary, cmp: compareEval?.skinfolds?.midaxillary, unit: 'mm', inverseGood: true },
                      { label: 'Supra-il├¡aca', cur: currentEval?.skinfolds?.suprailiac, cmp: compareEval?.skinfolds?.suprailiac, unit: 'mm', inverseGood: true },
                    ...
                    {activeTab === 'circunferencias' && renderTableContent([
                      ...
                      { label: 'Bra├ºo Dir.', cur: currentEval?.circumferences?.armRight, cmp: compareEval?.circumferences?.armRight, unit: 'cm' },
                      { label: 'Bra├ºo Esq.', cur: currentEval?.circumferences?.armLeft, cmp: compareEval?.circumferences?.armLeft, unit: 'cm' },
          ...
          {currentMetrics && (
            <div className="footer-info">
              <span className="info-icon">Ôôÿ</span>
              Os dados apresentados s├úo referentes ├á avalia├º├úo selecionada. A tend├¬ncia mostra a diferen├ºa para a avalia├º├úo de compara├º├úo.
            </div>
          )}
            ...
            <div className="delete-modal-overlay">
              <div className="delete-modal-content">
                ...
                <p className="delete-modal-text">
                  Tem certeza que deseja excluir <strong>{athlete.name}</strong>? Esta a├º├úo remover├í tamb├®m todas as avalia├º├Áes dele e n├úo poder├í ser desfeita.
                </p>
